...
export default function PerformanceGraph({ data }: { data: any[] }) {
  ...
  const formattedData = targetData.map(d => ({
    date: d.date,
    value: Number(d.total_value)
  }))

  const minVal = Math.min(...formattedData.map(d => d.value)) * 0.95
  const maxVal = Math.max(...formattedData.map(d => d.value)) * 1.05
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={formattedData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
          ...
          <Tooltip 
            ...
            itemStyle={{ color: '#fff', fontWeight: 600, fontSize: '16px' }}
